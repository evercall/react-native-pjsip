...
export default class Endpoint extends EventEmitter {
    ...
    static updateStunServers(accountId, stunServerList) {
        return new Promise((resolve, reject) => {
     ...
     * @returns {Promise}
     */
    static changeNetworkConfiguration(configuration) {
        return new Promise((resolve, reject) => {
            PjSipModule.changeNetworkConfiguration(configuration, (successful, data) => {
                if (successful) {
                    resolve(data);
                }
                else {
     ...
     * @returns {Promise}
     */
    static changeServiceConfiguration(configuration) {
        return new Promise((resolve, reject) => {
     ...
     * @param configuration
     */
    // static replaceAccount(account: Account, configuration: AccountConfiguration): void {
     ...
     * @returns {Promise}
     */
    static registerAccount(account, renew = true) {
        return new Promise((resolve, reject) => {
     ...
     */
    static makeCall(account, destination, callSettings, msgData) {
        const newDestination = this._normalize(account, destination);
        return new Promise((resolve, reject) => {
            NativeModules.PjSipModule.makeCall(account.getId(), newDestination, callSettings, msgData, (successful, data) => {
                if (successful) {
     ...
     * @returns {Promise}
     */
    static xferCall(account, call, destination) {
        const newDestination = this._normalize(account, destination);
        return new Promise((resolve, reject) => {
            NativeModules.PjSipModule.xferCall(call.getId(), newDestination, (successful, data) => {
                if (successful) {
                    resolve(data);
                }
                else {
     ...
     * @returns {Promise}
     */
    static xferReplacesCall(call, destCall) {
        return new Promise((resolve, reject) => {
     ...
     * @returns {Promise}
     */
    static redirectCall(account, call, destination) {
        const newDestination = this._normalize(account, destination);
        return new Promise((resolve, reject) => {
            NativeModules.PjSipModule.redirectCall(call.getId(), newDestination, (successful, data) => {
                if (successful) {
                    resolve(data);
                }
                else {
    ...
    _onConnectivityChanged(available) {
         ...
         * @property bool available True if connectivity matches current Network settings,
         * otherwise false.
         */
     ...
     * @private
     */
    static _normalize(account, destination) {
        let returnString;
        if (!destination.startsWith('sip:')) {
            ...
            returnString = `sip:${destination}@${realm}`;
        }
        return returnString;
    }
}
