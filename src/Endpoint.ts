...
export default class Endpoint extends EventEmitter {
   ...
   * @returns {Promise}
   */
  static registerAccount(account: Account, renew: boolean = true): Promise<any> {
    return new Promise((resolve, reject) => {
  ...
  static makeCall(
    ...
    return new Promise((resolve, reject) => {
      NativeModules.PjSipModule.makeCall(
        account.getId(), newDestination, callSettings, msgData, (successful, data) => {
          if (successful) {
  ...
  static changeOrientation(orientation: (
    ...
    if (orientations.indexOf(orientation) === -1) {
      throw new Error(`Invalid ${JSON.stringify(orientation)} device orientation, but expected ${orientations.join(
        ', ',
      )} values`)
    }
